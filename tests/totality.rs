...
use fulcrum::{Capacity, Fleet, HotToCold, Linfty, MachineId, Mass, Neutral, Remove, Safe};

fn one_machine(load: u64) -> Fleet<1> {
    let mut f = Fleet::new();
    f.add_machine(MachineId(1), Capacity([100]), Mass([load]));
    f
}
...
fn remove_apply_returns_safe_directly() {
    ...
    assert_eq!(safe.fleet().load(MachineId(1)), Some(Mass([40])));
}
...
fn hot_to_cold_apply_returns_safe_directly() {
    ...
    assert_eq!(safe.fleet().load(MachineId(1)), Some(Mass([60])));
    assert_eq!(safe.fleet().load(MachineId(2)), Some(Mass([50])));
}
...
fn neutral_apply_returns_safe_directly() {
    ...
    let safe = m.apply(safe);
    assert_eq!(safe.fleet().load(MachineId(1)), Some(Mass([40])));
    assert_eq!(safe.fleet().load(MachineId(2)), Some(Mass([60])));
}
...
fn neutral_witness_rejects_unequal_loads() {
    let mut f: Fleet<1> = Fleet::new();
    f.add_machine(MachineId(1), Capacity([100]), Mass([50]));
    f.add_machine(MachineId(2), Capacity([100]), Mass([51]));
